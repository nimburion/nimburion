...
func TestMetrics_RegisterInCustomRegistryOnly(t *testing.T) {
	...
	metrics.record("get", nil)
	metricsAgain.record("set", nil)

	assertMetricPresent(t, registry.Gatherer(), "nimburion_session_redis_operations_total")
	assertMetricAbsent(t, prometheus.DefaultGatherer, "nimburion_session_redis_operations_total")
}
