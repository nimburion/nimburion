...
func TestCreated(t *testing.T) {
	req := httptest.NewRequest(http.MethodPost, "/test", nil).WithContext(context.WithValue(context.Background(), middleware.RequestIDKey, "req-789"))
	mockCtx := &mockContext{request: req, response: newMockResponseWriter()}
...
func TestMapError(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			ctx:  context.WithValue(context.Background(), middleware.RequestIDKey, "req-i18n"), wantStatus: http.StatusBadRequest, wantErrorCode: "validation_error", wantMessage: "validation.email_required", wantRequestID: "req-i18n",
		},
		{
			name: "validation error", err: NewValidationError("invalid input", map[string]interface{}{"field": "email"}),
			ctx: context.WithValue(context.Background(), middleware.RequestIDKey, "req-123"), wantStatus: http.StatusBadRequest, wantErrorCode: "validation_error", wantMessage: "invalid input", wantRequestID: "req-123", wantHasDetails: true,
		},
		{
			name: "unknown error type", err: errors.New("some random error"),
			ctx: context.WithValue(context.Background(), middleware.RequestIDKey, "req-jkl"), wantStatus: http.StatusInternalServerError, wantErrorCode: "internal_server_error", wantMessage: "an unexpected error occurred", wantRequestID: "req-jkl",
		},
		{
			name: "jobs family error", err: jobs.ErrConflict,
			ctx: context.WithValue(context.Background(), middleware.RequestIDKey, "req-jobs"), wantStatus: http.StatusConflict, wantErrorCode: "conflict", wantMessage: jobs.ErrConflict.Error(), wantRequestID: "req-jobs",
		},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
