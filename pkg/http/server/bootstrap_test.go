...
func TestBuildHTTPServers_RequiresManagementRouterWhenEnabled(t *testing.T) {
	cfg := config.DefaultConfig()
	cfg.Management.Enabled = true
	...
	if err == nil || err.Error() != "management router is required when management server is enabled" {
		t.Fatalf("expected explicit management router error, got %v", err)
	}
	if servers != nil {
		t.Fatalf("expected no servers result when management router is missing")
	}
}
