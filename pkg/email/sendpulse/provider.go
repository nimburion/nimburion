// Package sendpulse provides an email provider backed by the SendPulse API.
package sendpulse
...
type (
	// Config configures the SendPulse email provider.
	Config = emailconfig.TokenConfig
	// Provider sends email through the SendPulse API.
	Provider struct {
		cfg        Config
...
// New constructs a SendPulse-backed email provider.
...
// Send delivers message using the configured SendPulse account.
