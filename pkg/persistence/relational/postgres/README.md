...
PostgreSQL adapter for the relational persistence family.

Owns production-ready PostgreSQL connectivity, pooling, health checks, graceful shutdown, and transaction management.
...
```bash
go test ./pkg/persistence/relational/postgres -short
```
...
```bash
go test ./pkg/persistence/relational/postgres -run TestProperty14
```
...
```bash
go test ./pkg/persistence/relational/postgres -run TestPostgreSQLAdapter_Integration
```
...
- MySQL portability logic above the relational family
- migration orchestration outside adapter concerns

## Status

Target-state adapter package.

## See Also

- [Relational Contracts](/Users/giuseppe/Workspace/github.com/nimburion/nimburion-refactoring-worktree/pkg/persistence/relational/README.md)
