...
import (
	...
	"github.com/nimburion/nimburion/pkg/persistence/relational"
)
...
func Example() {
	...
	userRepo := relational.NewGenericCrudRepository[User, int64](
		db,
		"users",
		"id",
		&UserMapper{},
	...
	opts := relational.QueryOptions{
		Filter: relational.Filter{
			"name": "John Doe",
		},
		Sort: relational.Sort{
			Field: "email",
			Order: relational.SortAsc,
		},
		Pagination: relational.Pagination{
			Page:     1,
			PageSize: 10,
		},
	}
	...
	// Count users
	count, err := userRepo.Count(ctx, relational.Filter{"name": "John Doe"})
	if err != nil {
		log.Fatal(err)
	}
