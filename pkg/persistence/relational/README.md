# `pkg/persistence/relational`

Relational persistence family root with generic SQL repository contracts and helpers.

## Overview

This package provides the SQL-oriented abstraction layer between business logic and relational data stores. It uses Go generics to provide type-safe CRUD operations while keeping relational concerns separate from document, key-value, search, and object families.
...
5. **Adapter Pattern**: Concrete implementations are provided by relational adapters such as [`pkg/persistence/relational/postgres`](/Users/giuseppe/Workspace/github.com/nimburion/nimburion-refactoring-worktree/pkg/persistence/relational/postgres/README.md) and [`pkg/persistence/relational/mysql`](/Users/giuseppe/Workspace/github.com/nimburion/nimburion-refactoring-worktree/pkg/persistence/relational/mysql/README.md)
...
- use this package for relational contracts, pagination, transaction semantics, optimistic locking, and SQL-shaped generic CRUD helpers
- use [`pkg/persistence/relational/postgres`](/Users/giuseppe/Workspace/github.com/nimburion/nimburion-refactoring-worktree/pkg/persistence/relational/postgres/README.md) and [`pkg/persistence/relational/mysql`](/Users/giuseppe/Workspace/github.com/nimburion/nimburion-refactoring-worktree/pkg/persistence/relational/mysql/README.md) as backend adapters
- use [`pkg/persistence/relational/migrate`](/Users/giuseppe/Workspace/github.com/nimburion/nimburion-refactoring-worktree/pkg/persistence/relational/migrate/doc.go) for SQL migration execution

## Non-Goals

- document-store contracts
- generic legacy umbrella ownership
- transport or HTTP concerns

## Testing

- fast tests cover CRUD helpers, transactions, optimistic locking, and repository contracts
- adapter-specific integration and property tests live in each backend package

## Status

Target-state family root.
...
For document databases (MongoDB and DynamoDB), use `pkg/persistence/document`.

It provides:
- document-oriented repository contracts (`Reader`/`Writer`/`Repository`)
- backend executors:
  - `MongoDBExecutor` (on top of `pkg/persistence/document/mongodb`)
  - `DynamoDBExecutor` (on top of `pkg/persistence/keyvalue/dynamodb`)
...
3. **Context Propagation**: All operations accept context for cancellation and timeout support
4. **Domain-Driven**: Interfaces are defined by domain needs, not database capabilities
5. **Adapter Pattern**: Concrete implementations are provided by role- or family-specific adapter packages under `pkg/persistence/*`, `pkg/cache/*`, or `pkg/session/*`
