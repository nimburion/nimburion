...
func TestMetrics_RegisterInCustomRegistryOnly(t *testing.T) {
	...
	metrics.record("acquire", nil)
	metricsAgain.record("renew", nil)

	assertMetricPresent(t, registry.Gatherer(), "nimburion_coordination_redis_operations_total")
	assertMetricAbsent(t, prometheus.DefaultGatherer, "nimburion_coordination_redis_operations_total")
}
