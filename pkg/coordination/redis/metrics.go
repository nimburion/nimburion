...
func NewMetrics(registry *frameworkmetrics.Registry) (*Metrics, error) {
	...
	if err := registry.Register(opsTotal); err != nil {
		...
		if !errors.As(err, &alreadyRegistered) {
			return nil, coreerrors.WrapConstructorError("NewMetrics", fmt.Errorf("register coordination redis metrics: %w", err))
		}
		existing, ok := alreadyRegistered.ExistingCollector.(*prometheus.CounterVec)
		if !ok {
			return nil, coreerrors.WrapConstructorError("NewMetrics", fmt.Errorf("register coordination redis metrics: existing collector has type %T", alreadyRegistered.ExistingCollector))
		}
		opsTotal = existing
	}
