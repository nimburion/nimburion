...
// NewFromConfig builds an event bus adapter from config without schema validation.
func NewFromConfig(cfg config.EventBusConfig, log logger.Logger) (eventbus.EventBus, error) {
	return NewFromConfigWithValidation(cfg, config.KafkaValidationConfig{}, log)
}

// NewFromConfigWithValidation builds an event bus adapter from config and optionally wraps it with schema validation.
func NewFromConfigWithValidation(
